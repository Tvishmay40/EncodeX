...
function RobotArmViz({ position, color }) {
  ...
  const sR = toR((-65 + (x / 100) * 130) - 90);
  const eR = toR((-65 + (x / 100) * 130) - 90 + (-85 + (y / 100) * 105));
  ...
  return (
    ...
    >
      ...
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", position: "relative", zIndex: 1, flexShrink: 0 }}>
        <div>
          <div style={{ fontSize: 11, fontWeight: "bold", letterSpacing: "0.18em", textTransform: "uppercase", color: accent }}>
            {data.type}
          </div>
          <div style={{ fontSize: 9, letterSpacing: "0.12em", textTransform: "uppercase", color: "#475569" }}>
            {machineKey}
          </div>
        </div>
        <div style={{
...
function getSimulatedData(t) {
  return {
    ...
    machines: {
      cnc_1: {
        type: "CNC", status: "active",
        temp_c: Math.round(44 + Math.sin(t) * 3),
        power_w: Math.round(1200 + Math.sin(t * 1.3) * 80),
        vibration_hz: Math.round(12 + Math.sin(t * 2) * 2),
        position: { x: 50 + Math.sin(t) * 40, y: 50 },
      },
      printer_1: {
        type: "3D Printer", status: "active",
        temp_c: Math.round(210 + Math.sin(t * 0.5) * 4),
        power_w: Math.round(340 + Math.cos(t) * 20),
        vibration_hz: 4,
        position: { x: 50 + Math.cos(t * 1.5) * 40, y: 70 },
      },
      lathe_1: {
        type: "Lathe", status: "active",
        temp_c: Math.round(78 + Math.cos(t * 0.8) * 5),
        power_w: Math.round(860 + Math.sin(t * 1.1) * 60),
        vibration_hz: Math.round(30 + Math.sin(t * 3) * 4),
        position: { x: 50 + Math.sin(t * 2) * 35, y: 50 },
      },
      robot_1: {
        type: "Robotic Arm", status: "active",
  ...
  return (
    <div style={{
      ...
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 10, flexShrink: 0 }}>
        <div>
          <div style={{ fontSize: 9, letterSpacing: "0.35em", textTransform: "uppercase", color: "#475569" }}>
            Industrial Control System
          </div>
          <div style={{ fontSize: 14, fontWeight: "bold", letterSpacing: "0.1em", textTransform: "uppercase", color: "#00ffe5" }}>
